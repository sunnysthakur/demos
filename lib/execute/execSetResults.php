...
smarty_assign_tsuite_info($smarty,$_REQUEST,$db,$tree_mgr,$tcase_id,$args->tproject_id);
	...
	/* New code implementations starts */
	
	$query		=	"SELECT b.keyword FROM testcase_keywords a JOIN keywords b ON a.keyword_id = b.id where a.testcase_id = '".$tcase_id."'";
	// $rs12	=	$db->exec_query($query);
	$a_keywords	= $db->get_recordset($query);
	$t_keyword	=	'';
	if(!empty($a_keywords)){
		foreach($a_keywords as $k => $val){
			$t_keyword	.=	$a_keywords[$k]['keyword'].", ";
		}
	}
	if(!empty($t_keyword)){
		$t_keyword	=	rtrim($t_keyword,", ");
	}
	$gui->t_keyword	=	$t_keyword;
	
	/* New code implementations ends */
