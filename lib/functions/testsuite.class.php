...
//require_once('../../third_party/phpexcel/write_excel/Worksheet.php');
//require_once('../../third_party/phpexcel/write_excel/Workbook.php');
...
class testsuite extends tlObjectWithAttachments
{
	...
	// comment by amit var $export_file_types = array("XML" => "XML" , "XLS" => "XLS" );
...
/*
	  ...
	  Done By  : Amit Sharma
	  function : exportTestSuiteDataToXLS
...
/*
   Done By     : Amit Sharma
   Description : downloading headers for excel
...
function exportTestSuiteDataToXLS($container_id,$tproject_id,$optExport = array())
{
		...
		if($doRecursion)
		{
			...
			if( isset($optExport['KEYWORDS']) && $optExport['KEYWORDS'])
			{
				...
				if ($kwMap)
				{
					//$kwXML .= $this->xlsWriteLabel(0,0,$keywordMgr->toXMLString($kwMap,true));
			...
			$xmlTC .= $this->xlsWriteLabel(0,0,htmlspecialchars($tsuiteData['name']));

			$xmlTC .= $this->xlsWriteLabel(0,1,$tsuiteData['node_order']);

			$xmlTC .= $this->xlsWriteLabel(0,2,$tsuiteData['details']);*/
		...
		if( !is_null($childNodes) )
		{
		    ...
		    for($idx = 0;$idx < $loop_qty;$idx++)
		    {
		    	...
		    	if ($doRecursion && $nTable == 'testsuites')
		    	{
					...
					/*$xmlTC .= $this->xlsWriteNumber($xlsRow,0,$cNode['id']);

					$xmlTC .= 	$this->xlsWriteLabel($xlsRow,1,$tproject_id);

					$xmlTC .= $this->xlsWriteLabel($xlsRow,2,$optExport);*/
					

		    		$xmlTC .= $this->exportTestSuiteDataToXLS($cNode['id'],$tproject_id,$optExport);
		    	}
		    	else if ($nTable == 'testcases')
		    	{
		    		...
		    		$xmlTC .= $tcase_mgr->exportTestCaseDataToXLS($cNode['id'],testcase::LATEST_VERSION,
