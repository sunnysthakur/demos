...
/* 
  Done By : Amit Sharma
  Description : added xml and xls in array for importing in xml and xls format
*/
	...
	{ 
		...
		if (move_uploaded_file($source, $dest))
		{
			...
			switch($args->importType)
			{
				...
				case 'XML':
				  ...
				  /* 
					Done By : Amit Sharma
					Description : Import test case and suite with XLS
		...
		if($gui->file_check['status_ok'] && $pimport_fn)
		{
			...
			if($pimport_fn == 'importTestSuiteDataFromSpreadsheet' || $pimport_fn == 'importTestCaseDataFromSpreadsheet'){
				if($_FILES['uploadedFile']['type'] == 'application/octet-stream' || $_FILES['uploadedFile']['type'] == 'application/vnd.ms-excel'){
					tLog('Check is Ok.');
					$opt = array();
					$opt['useRecursion'] = $args->useRecursion;
					$opt['importIntoProject'] = $args->bIntoProject;
					...
					// echo "<pre>";print_r($gui->resultMap);
                            ...
                            $userID,$kwMap,$duplicatedLogic = array('hitCriteria' => 'name', 'actionOnHit' => null))
{
	...
	if(is_null($messages))
	{
	   ...
	   // $messages['start_feedback'] = $messages['start_warning'] . "\n" . $messages['testlink_warning'] . "\n";
		...
		// added by amit 
		// date : 12th Oct 2011
		$safeSizeCfg->testcase_name=($fieldSizeCfg->testcase_name) * 1.0;
...
/*
  Description :code for multiple test suite
  Done by : amit sharma
...
{	
	...
	define('IDX_COL_ACTIONS',6);
	define('IDX_COL_EXPRESULTS',7);
	define('IDX_COL_KEYWORDS',8);
	...
	for($idx = FIRST_DATA_ROW; $idx <= $xls_row_qty; $idx++ )
	{
		...
		*/
		...
		$a_steps			=	explode("||",$actions);
		$a_expresults		=	explode("||",$expresults);
		
		/* echo "<pre>";
			print_r($a_steps);
		echo "</pre>";
		echo "<pre>";
			print_r($a_expresults);
		echo "</pre>";
		...
		if(!empty($a_steps)){
			foreach($a_steps as $ky => $stp){
				$stp	=	trim($stp);
				if(!empty($stp) && $stp != ''){
					$stp 	=	nl2p(htmlspecialchars($stp));
					$expts	=	@$a_expresults[$ky];
					if(!empty($expts)){
						$expts = nl2p(htmlspecialchars($expts));
					}
					...
					fwrite($xmlFileHandle,"<actions><![CDATA[" . $stp . "]]></actions>\n");
					fwrite($xmlFileHandle,"<expectedresults><![CDATA[" . $expts . "]]></expectedresults>\n");
		...
		if(!empty($keywords)){
			...
			foreach($a_keywords as $kk => $vv){
				if(!empty($vv)){
					fwrite($xmlFileHandle,"<keyword name=" . '"' . $vv. '"'.">\n");
		...
		$Preconditions	=	"<pre>".$Preconditions."</pre>";
...
function import_testcase_csv($file)
{
		...
		while (($data = fgetcsv($handle, 1000, ',')) !== FALSE)
        {
			 ...
			 for ($i = 0; $i < $cols; $i++)
             {
				 ...
				 // }
				   ...
				   if($titles[$i]=="testcase"){
					$r .= "<{$titles[$i]}"." ".'name='.'"'.$data[$i].'"'.">\n";
                    @$ctrlclosechksuite=2;
					}
				  if($titles[$i]=="summary"){
                  $r .= "<{$titles[$i]}>";
				 ...
				 if($titles[$i]=="keyword"){
				  ...
				  if(@$data[$i]!=""){
                  @$seprated_comma=explode(",",$data[$i]);
				  @$count_keys=count($seprated_comma);
				  
                  $r .= "<keywords>\n";
				           $key_i=0;
				  			for($key_i=0;$key_i<=count($seprated_comma);$key_i++){
							 if(@$seprated_comma[$key_i]!=""){ 
								 $r .= "<{$titles[$i]}"." ".'name='.'"'.$seprated_comma[$key_i].'"'.">\n";
								  $r .= "<notes><![CDATA[]]></notes>\n";
									$r .= "</{$titles[$i]}>\n";
								}
							}
...
function importtestcasecsv(&$db,$fileName,$parentID,$tproject_id,
                                           $userID,$useRecursion,$importIntoProject = 0)
{
...
/*
   Done By : Amit Sharma
   Description : import test case data from excel
...
function create_xml_tcspec_from_xls($xls_filename,$xml_filename)
{
	...
	define('IDX_COL_STEPS',4);
	define('IDX_COL_EXPRESULTS',5);
	define('IDX_COL_PRECONDITIONS',3);
	define('IDX_COL_KEYWORDS',6);
	...
	for($idx = FIRST_DATA_ROW; $idx <= $xls_row_qty; $idx++ )
	{
		...
		$steps = convert_special_char($xls_rows[$idx][IDX_COL_STEPS]);
	    // $steps = nl2p1(htmlspecialchars($steps));
		$expresults = convert_special_char($xls_rows[$idx][IDX_COL_EXPRESULTS]);
		// $expresults = nl2p(htmlspecialchars($expresults));
		
		$a_steps			=	explode("||",$steps);
		$a_expresults		=	explode("||",$expresults);
